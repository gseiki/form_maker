...
import os # Imported for "script_dir", "module_dir", and "creds_dir" variables.
...
def main():
   ...
   if ic_validated and ds_validated:
      ...
      validate_google_sheet.set_delimiter("\t") # use the same delimiter as ic_header because you're comparing
                                                # dataframe to ic_header.
      validate_google_sheet.set_header(ic_header)
      old_student_df = google.import_sheet_to_dataframe(scanner_spreadsheet_id, scanner_sheet_name, "old_student_df")
      ...
      if sheet_empty:
         ...
         google_df_validated = validate_google_sheet.data_frame(old_student_df)
      else:
         google_df_validated = validate_google_sheet.data_frame(old_student_df)
      ...
      if google_df_validated:
         ...
         if not the_same:
            ...
            new_survey_df = ic_dob_df[~ic_dob_df['State ID'].isin(old_student_df['State ID'])]
            new_survey_df.reset_index(drop=True, inplace=True)
            new_students_df = ic_df[~ic_df['State ID'].isin(old_student_df['State ID'])]
            ...
            if new_student_df_is_empty:
               ...
               print("but roster changes were detected, and necessary Google Sheets were updated!")
               google.export_df_to_sheet(scanner_spreadsheet_id, scanner_sheet_name, ic_df)
               google.export_df_to_sheet(roster_spreadsheet_id, roster_sheet_name, roster_df)
            else:
               print("Found new students! Creating forms and updating Google Sheets!")
               magick = Magick()
...
## Uncomment the next three lines if you want this script automatically print forms of any new students.
#               magick.print_trug()
#               magick.print_spvr()
#               magick.print_survey()
